...
public class CustomHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public static boolean isProxyRequest(Class<?> clazz) {
		return BaseProxyRequest.class.isAssignableFrom(clazz);
	}

	public static boolean isSessionUser(MethodParameter parameter, Class<?> clazz) {
	...
	public boolean supportsParameter(final MethodParameter parameter) {
		...
		return isProxyRequest(clazz) || isSessionUser(parameter, clazz);
	}

	@Override
	public Object resolveArgument(final MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
		final Class<?> clazz = parameter.getParameterType();
		if (isProxyRequest(clazz)) { // ProxyRequest
			HttpServletRequest request = webRequest.getNativeRequest(HttpServletRequest.class);
			HttpServletResponse response = webRequest.getNativeResponse(HttpServletResponse.class);
			BaseProxyRequest proxyRequest = (BaseProxyRequest) BeanUtils.instantiateClass(clazz);
			return proxyRequest.init(request, response, mavContainer.getModel());
		} else if (isSessionUser(parameter, clazz)) { // @SessionUser User
			final HttpSession session = webRequest.getNativeRequest(HttpServletRequest.class).getSession(false);
			return session == null ? null : session.getAttribute(Context.internal().getUserSessionAttr());
		}
		Assert.state(mavContainer != null, "ModelAttributeMethodProcessor requires ModelAndViewContainer");
		Assert.state(binderFactory != null, "ModelAttributeMethodProcessor requires WebDataBinderFactory");
		String name = parameter.getParameterName(); // Conventions.getVariableNameForParameter(parameter);
		/*
		...
		*/
		...
		if (bindingResult == null) {
			...
			if (binder.getTarget() != null) {
				binder.setFieldDefaultPrefix(name + '.');
	 ...
	 * <p>
	 * The default implementation checks for {@code @javax.validation.Valid}, Spring's {@link Validated}, and custom annotations whose name starts with "Valid".
	 *
		...
		for (Annotation ann : parameter.getParameterAnnotations()) {
			...
			if (validationHints != null) {
				for (Validator validator : binder.getValidators()) {
					if (validator instanceof SmartValidator sv) {
						try {
							sv.validateValue(targetType, fieldName, value,
