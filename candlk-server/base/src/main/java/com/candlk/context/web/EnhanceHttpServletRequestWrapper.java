...
import com.candlk.common.context.RequestContextFilter;
...
public class EnhanceHttpServletRequestWrapper extends HttpServletRequestWrapper {
	...
	static AES aes;
	/** 是否拒绝不可信的请求时间 */
	public static transient boolean denyUntrustedRequestTime;
	// ** 第三方回调请求路径的后缀 */
	...
	public static String resolve(@Nullable String appIdHeader, @Nullable HttpServletRequest request) {
		...
		try {
			...
			if (diff > 10 * 60) { // 前后差异不能超过 10 分钟
				log.warn("客户端时间异常：AppId={}，CID={}，diff={} s", timestampInSecs + "/" + appId, request == null ? null : request.getHeader("cid"), (nowInSecs - timestampInSecs));
				if (denyUntrustedRequestTime) {
					throw new ErrorMessageException("请检查系统时间是否设置有误！", MessagerStatus.UNTRUSTED).report();
				}
			}
			return appId;
		} catch (Exception e) {
