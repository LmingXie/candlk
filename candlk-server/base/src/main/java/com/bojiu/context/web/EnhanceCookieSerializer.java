...
/**
 * 支持动态自定义 cookieName 和 cookiePath 的 Cookie 序列化器
 *
 * @see org.springframework.session.web.http.DefaultCookieSerializer
 */
public class EnhanceCookieSerializer implements CookieSerializer {

	private static final Log logger = LogFactory.getLog(EnhanceCookieSerializer.class);
	...
	/**
	 * @deprecated 不能直接调用该属性，请使用 {@link #cookieName(HttpServletRequest)} 替代
	 */
	@Deprecated
	...
	protected boolean useHttpOnlyCookie = true;
	...
	protected Pattern domainNamePattern;

	protected String jvmRoute;

	protected boolean useBase64Encoding = true;

	protected String rememberMeRequestAttribute;
	...
	public List<String> readCookieValues(HttpServletRequest request) {
		...
		if (cookies != null && cookies.length > 0) {
			final String cookieName = cookieName(request);
			for (Cookie cookie : cookies) {
				if (cookieName.equals(cookie.getName())) {
					sessionId = (this.useBase64Encoding ? base64Decode(cookie.getValue()) : cookie.getValue());
					if (sessionId == null) {
						continue;
					}
		...
		// 如果 请求头 有值，且不等于 Cookie 中的 sessionId
		if (StringUtil.notEmpty(auth) && (sessionId == null || !auth.startsWith(sessionId))) {
			// "GGXz-eaxLpRLRuVNvoDUS"
			int pos = auth.indexOf(DefaultAutoLoginHandler.sessionIdSep, 20);
			if (pos > 0) { // UUID 长度是 36 / sessionId 长度是 21
				String headerSessionId = auth.substring(0, pos);
				if (sessionId != null) {
					return Arrays.asList(sessionId, headerSessionId);
				}
	...
	public static String cookieName(HttpServletRequest request) {
		return switch (MemberType.parseFrom(request)) {
			case ADMIN -> "PID";
			case MERCHANT -> "MID";
			case AGENT -> "AID";
			default -> "UID";
	...
	public void writeCookieValue(CookieValue cookieValue) {
		...
		sb.append(cookieName(request)).append('=');
	...
	private String base64Encode(String value) {
		final byte[] bytes = JavaUtil.STRING_CODER.applyAsInt(value) == JavaUtil.LATIN1
				? JavaUtil.STRING_VALUE.apply(value)
				: value.getBytes();
		return Common.base64ToString(bytes, true);
	}
	...
	private void validateDomain(String domain) {
		...
		while (i < chars.length) {
			prev = cur;
			cur = chars[i];
			if (!domainValid.get(cur) || ((prev == '.' || prev == -1) && (cur == '.' || cur == '-')) || (prev == '-' && cur == '.')) {
